...
function imageMarker(ref, perceptionActive) {
  const namePart = ref.name ? ` name=${JSON.stringify(ref.name)}` : ''
  const action = perceptionActive
...
function sanitizeBlocks(content, perceptionActive) {
  return asArray(content).flatMap((block) => {
    if (block?.type === 'image') {
      const ref = normalizedImageRef(block.attachment)
      const marker = ref
        ? imageMarker(ref, perceptionActive)
        : '<dsh_media kind="image" state="invalid_attachment_ref" />'
      return [{ type: 'text', text: marker }]
    }
    if (block?.type === 'tool-result') {
      return [{ ...block, content: sanitizeBlocks(block.content, perceptionActive) }]
    }
    return [block]
  })
}

export function sanitizeMessagesForTextModel(messages, perceptionActive) {
  return asArray(messages).map((message) => {
    const content = sanitizeBlocks(message.content, perceptionActive)
...
export function rewriteHistoricalImages(agent) {
  ...
  for (const seq of nodes) {
    ...
    const [replacement] = sanitizeMessagesForTextModel([message], true)
...
export function projectAssembly(assembled, events, config = {}, mediaActive = false) {
  ...
  if (config.holdThatBootstrap === true && !hasDurableToolCall(events)) {
    const selected = bootstrapNames(tools, config, mediaActive)
    return { ...assembled, tools: tools.filter(tool => selected.has(tool.name)) }
  }
  return {
    ...assembled,
    tools: tools.filter(tool => tool.name !== TOOL_NAME || mediaActive),
  }
}
...
export function apply(ctx, config = {}) {
  ...
  disposers.push(ctx.tools.register(toolDefinition(ctx, state, config)))
  ...
  disposers.push(ctx.on('tools/post-execute', async (exec, result, next) => {
    ...
    return { ...decision, content: sanitizeBlocks(content, true) }
  }))
  // Definition-owned finalizers run after post-execute. Observe their frozen
  // output so the next system-prompt assembly still exposes perceive_media;
  // pre-step surface migration removes any raw image block before the request.
  disposers.push(ctx.on('tools/result', (exec, result) => {
    if (config.sanitizeImages === false || !exec.agent) return
  ...
  disposers.push(ctx.on('agent/session-start', ({ agent }) => {
    ...
    const migration = rewriteHistoricalImages(agent)
  ...
  disposers.push(ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    return projectAssembly(assembled, agent.session?.events, config, activeForAgent(agent, state.pending))
  }))
  disposers.push(ctx.on('agent/pre-step', async ({ agent }, next) => {
    if (config.migrateHistoricalImages !== false && config.sanitizeImages !== false) {
      const migration = rewriteHistoricalImages(agent)
      if (migration.rewritten > 0 && migration.latestRefs.length > 0) {
    ...
    if (refs.length === 0) return decision
    state.pending.set(agent, refs)
    return {
      ...decision,
      messages: sanitizeMessagesForTextModel(decision.messages, true),
    }
  }))
  return () => {
