...
- 文件工具收到的 `/mnt/<drive>/...` 会映射回同一 Windows 盘符路径。
- WSL 内使用原生 Linux `PATH`，所以 `node`、`git` 和其他命令均来自 WSL。
- 前台与后台任务都复用 DSH 原有的进程、输出和超时管理。

因此模型可在 Bash 和 `read`、`write`、`edit` 等文件工具之间复用同一个绝对
路径。例如 `/mnt/d/projfiles/a.py` 会落到 `D:\\projfiles\\a.py`，不会误写成
`D:\\mnt\\d\\projfiles\\a.py`。相对路径保持不变，并以会话工作目录为基准。
...
- 用继承原 `fs-sandbox` 的路径桥接后端统一 `/mnt/<drive>/...` 文件路径；
...
该插件把 shell 工具执行切换到 WSL，并为 Windows 文件工具增加路径名称空间桥接。
旧 DSH 的 UI、会话存储、文件工具实现、模型路由和 Node 主进程仍运行在 Windows。
需要整个 DSH 都运行在 Linux 时，应在 WSL
内安装并启动 DSH。
