...
let readImageDefinition
const observedImages = []
const readImageCtx = {
  tools: {
    register(definition) {
      readImageDefinition = definition
      return () => { readImageDefinition = undefined }
    },
  },
  attachments: {
    imageLimits: {
    ...
    async saveImage({ data, mediaType, name }) {
      check(data.byteLength, 68)
      check(mediaType, 'image/png')
      check(name, 'fixture.png')
      return {
  ...
  fs: {
    async resolve(filePath, options) {
      check(filePath, 'fixture.png')
      check(options.cwd, 'C:\\fixture')
      return { displayPath: 'C:\\fixture\\fixture.png' }
    },
    async stat() { return { type: 'file', size: 68, version: 'fixture-v1' } },
    async readBytes() { return new Uint8Array(68) },
  },
  emit(name, target, observation) {
    observedImages.push({ name, target, observation })
  },
}
const disposeReadImage = applyReadImageBridge(readImageCtx)
check(readImageDefinition.name, 'read_image')
const readImageResult = await readImageDefinition.execute({ file_path: 'fixture.png' }, {
  signal: new AbortController().signal,
  agent: { session: { header: { cwd: 'C:\\fixture' } } },
})
check(readImageResult.image.attachmentId, 'fixture-image')
check(readImageDefinition.output.render({}, readImageResult)[1].type, 'image')
check(observedImages[0].name, 'fs/observed')
await assert.rejects(
  readImageDefinition.execute({ file_path: 'fixture.svg' }, { signal: new AbortController().signal }),
  /only accepts PNG\/JPEG\/WebP\/GIF/,
)
assertions += 1
disposeReadImage()
check(readImageDefinition, undefined)
...
const sanitizedDisabled = sanitizeMessagesForTextModel(nestedMessages, false, false)
check(sanitizedDisabled[0].content.some(block => block.type === 'image'), false)
ok(sanitizedDisabled[0].content[1].text.includes('visual perception is disabled'))
check(sanitizedDisabled[0].content[1].text.includes('call perceive_media'), false)
check(JSON.stringify(sanitizedDisabled).includes('image_url'), false)
...
const tools = [{ name: 'bash' }, { name: 'read' }, { name: 'edit' }, { name: 'read_image' }, { name: TOOL_NAME }]
...
check(projectAssembly({ tools }, [], {
  holdThatBootstrap: true, shellTools: ['bash', 'pwsh'], commonTools: ['read'], visionEnabled: false,
}, true).tools.map(tool => tool.name), ['bash', 'read'])
check(projectAssembly({ tools }, [{ type: 'tool/call' }], {}, false).tools.map(tool => tool.name), ['bash', 'read', 'edit', 'read_image'])
check(projectAssembly({ tools }, [{ type: 'tool/call' }], {}, true).tools.map(tool => tool.name), ['bash', 'read', 'edit', 'read_image', TOOL_NAME])
check(projectAssembly({ tools }, [{ type: 'tool/call' }], {
  visionEnabled: false,
}, true).tools.map(tool => tool.name), ['bash', 'read', 'edit', 'read_image'])
...
const disabledCtx = new FakeContext()
const disposeDisabled = apply(disabledCtx, {
  visionEnabled: false,
  holdThatBootstrap: true,
  shellTools: ['bash', 'pwsh'],
  commonTools: ['read'],
})
check(disabledCtx.definition, undefined, 'disabled vision must not register perceive_media')
const disabledAgent = { id: 'session-vision-disabled', session: { events: [] } }
disabledCtx.listener('agent/inbox/inserted')({ agent: disabledAgent, message: nestedMessages[0] })
const disabledAssembly = await disabledCtx.listener('system-prompt/assemble')(
  {}, { agent: disabledAgent }, async () => ({ tools }),
)
check(disabledAssembly.tools.map(tool => tool.name), ['bash', 'read'])
const disabledPreStep = await disabledCtx.listener('agent/pre-step')({ agent: disabledAgent }, async () => ({
  kind: 'enter', messages: nestedMessages,
}))
check(extractRawImageRefs(disabledPreStep.messages[0].content), [])
ok(disabledPreStep.messages[0].content[1].text.includes('visual perception is disabled'))
check(disabledPreStep.messages[0].content[1].text.includes('call perceive_media'), false)
disposeDisabled()
